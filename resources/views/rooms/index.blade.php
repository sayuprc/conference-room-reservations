    ...
    @if (!empty($rooms))
        <div class="mx-auto grid w-3/4 grid-cols-2 justify-center gap-4">
            @foreach ($rooms as $room)
                <div class="rounded-lg bg-white px-8 py-4 shadow-md">
                    <div class="mt-2">
                        <a href="{{ $room->detailUrl }}"
                            class="text-2xl font-bold text-gray-700 hover:text-gray-600 hover:underline">{{ $room->name }}</a>
                    </div>

                    <div class="mt-4">
                        <a href="{{ $room->detailUrl }}"
                            class="text-blue-600 hover:underline">もっと見る</a>
                    </div>
                </div>
            @endforeach
        </div>
